...
(function () {
    ...
    function formatFlightSearchParam(homeward) {
        var flightsSearchParam = _.reduce(
            $("#flights-search-form").serializeArray(),
            function (acc, item) {
    ...
    function reserveFlights(force) {
        ...
        for (var i in owCriteria) {
            $form.append($("<input>").attr("type", "hidden").attr("name", i).val(owCriteria[i]));
        }
        $form.append($("<input>").attr("type", "hidden").attr("name", "outwardDate").val(owCriteria.depDate));
        var hwCriteria = homewardFlightsTable.getSearchCriteria();
        $form.append($("<input>").attr("type", "hidden").attr("name", "homewardDate").val(hwCriteria.depDate));

        // モーダルを除去
        $("#login-modal").modal("hide");
        $(".modal-backdrop").remove();

        if (force) $(document).off("submit", "#flights-select-form", handlers.onSubmitFlightsSelectForm);
    ...
    function FlightsTable(selector) {
        ...
        var self = this,
            direction = selector.match(/outward/) ? "outward" : selector.match(/homeward/) ? "homeward" : "",
            $this = $(selector),
