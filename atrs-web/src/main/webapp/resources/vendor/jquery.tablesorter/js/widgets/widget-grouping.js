...
(function ($) {
    ...
    ts.grouping = {
        types: {
            ...
            date: function (c, $column, txt, part, group) {
                ...
                return part === "year"
                    ? time.getFullYear()
                    : part === "month"
                    ? wo.group_months[time.getMonth()]
                    : part === "day"
                    ? wo.group_months[time.getMonth()] + " " + time.getDate()
                    : part === "week"
                    ? wo.group_week[time.getDay()]
                    : part === "time"
                    ? ("00" + (hours > 12 ? hours - 12 : hours === 0 ? hours + 12 : hours)).slice(-2) +
                      ":" +
                      ("00" + time.getMinutes()).slice(-2) +
                      " " +
                      ("00" + wo.group_time[hours >= 12 ? 1 : 0]).slice(-2)
        ...
        update: function (table, c, wo) {
            ...
            c.$table.find("tr.group-hidden").removeClass("group-hidden").end().find("tr.group-header").remove();
                        ...
                        if ($rows.eq(rowIndex).is(":visible")) {
                            ...
                            if (ts.grouping.types[grouping[1]]) {
                                currentGroup = cache[rowIndex]
                                    ? ts.grouping.types[grouping[1]](
                                          c,
                                          c.$headers.filter('[data-column="' + column + '"]:last'),
                                          cache[rowIndex][column],
                                          /date/.test(groupClass) ? grouping[2] : parseInt(grouping[2] || 1, 10) || 1,
                                          group,
                                          lang
                                      )
                                    : currentGroup;
    ...
    ts.addWidget({
        ...
        remove: function (table, c, wo) {
            c.$table.off("click", "tr.group-header").off("pagerChange.tsgrouping").find(".group-hidden").removeClass("group-hidden").end().find("tr.group-header").remove();
        },
    });
})(jQuery);
