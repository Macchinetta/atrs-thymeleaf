...
(function ($) {
    ...
    var ts = $.tablesorter,
        output = (ts.output = {
            ...
            init: function (c) {
                c.$table.off(output.event).on(output.event, function () {
                    ...
                    $(this)
                        ...
                        .each(function () {
                                    ...
                                    if ($this.filter("[rowspan]").length) {
                                        ...
                                        for (row = 0; row < rowlen; row++) {
                                            ...
                                            tmpRow[rowIndex + row][cellIndex + col] = addSpanIndex
                                                ? wo.output_callbackJSON($this, txt, cellIndex + col) || txt + "(" + (cellIndex + col) + ")"
                                                : isHeader
                                                ? txt
                                                : dupe
                                                ? txt
                            ...
                            if ($this.css("display") !== "none") {
                                ...
                                tmpRow[rowIndex][cellIndex] = tmpRow[rowIndex][cellIndex] || output.formatData(wo, $this.attr(wo.output_dataAttrib) || $this.html(), isHeader);
                                cellIndex++;
                            }
                        });
                });
            ...
            process: function (c, wo) {
                ...
                $rows = /f/.test(wo.output_saveRows) ? $rows.not("." + (wo.filter_filteredRow || "filtered")) : /v/.test(wo.output_saveRows) ? $rows.filter(":visible") : $rows;
                ...
                if (outputJSON) {
                    ...
                    tmpData = output
                        .row2CSV(wo, wo.output_headerRows ? headers : [headers[len > 1 && wo.output_headerRows ? indx % len : len - 1]], outputArray)
