    ...
    var ts = $.tablesorter,
        ...
        tsColSel = (ts.columnSelector = {
            ...
            init: function (table, c, wo) {
                ...
                c.$table.off("refreshColumnSelector" + namespace).on("refreshColumnSelector" + namespace, function () {
            ...
            setupSelector: function (table, c, wo) {
                ...
                c.$table
                    ...
                    .each(function () {
                        ...
                        if ((isNaN(priority) && priority.length > 0) || state === "disable" || (wo.columnSelector_columns[colId] && wo.columnSelector_columns[colId] === "disable")) {
                            return true; // goto next
                        }

                        // set default state; storage takes priority
                        colSel.states[colId] =
                            saved && typeof saved[colId] !== "undefined"
                                ? saved[colId]
                                : typeof wo.columnSelector_columns[colId] !== "undefined"
                                ? wo.columnSelector_columns[colId]
                        ...
                        if ($container.length) {
                            ...
                            colSel.$checkbox[colId] = colSel.$wrapper[colId]
                                ...
                                .find("input")
                                .add(colSel.$wrapper[colId].filter("input"))
            ...
            updateBreakpoints: function (c, wo) {
                ...
                for (priority = 0; priority < 6; priority++) {
                    ...
                    if (breaks.length) {
                        ...
                        breakpts += tsColSel.queryBreak.replace(/\[size\]/g, wo.columnSelector_breakpoints[priority]).replace(/\[columns\]/g, breaks.join(","));
                    }
                }
                if (colSel.$style) {
                    colSel.$style.prop("disabled", true);
                }
                colSel.$breakpoints.prop("disabled", false).html(tsColSel.queryAll.replace(/\[columns\]/g, mediaAll.join(",")) + breakpts);
            },
