        ...
        init: function (table, thisWidget, c, wo) {
            ...
            $win.bind("scroll resize ".split(" ").join(namespace + " "), function () {
                var top = $attach.length ? $attach.offset().top : $win.scrollTop(),
                    ...
                    captionTop = wo.cssStickyHeaders_addCaption
                        ? $caption.outerHeight(true) + (parseInt(c.$table.css("padding-top"), 10) || 0) + (parseInt(c.$table.css("border-spacing"), 10) || 0)
                        : 0,
                    bottom = c.$table.height() - $thead.height() - (c.$table.find("tfoot").height() || 0) - captionTop,
                    deltaY = top - $thead.offset().top + (parseInt(c.$table.css("border-top-width"), 10) || 0) + (wo.cssStickyHeaders_offset || 0) + captionTop,
