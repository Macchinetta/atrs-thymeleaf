    ...
    var ts = ($.tablesorter = $.tablesorter || {}),
        ...
        bt = (ts.buildTable = function (tar, c) {
            ...
            if (!ts.buildTable.hasOwnProperty(typ) && typ !== "") {
                if (c.debug) {
                    ts.log("aborting build table widget, incorrect build type");
                }
                return false;
            }
            ...
            if (d instanceof jQuery) {
                ...
                $.ajax(wo.build_source)
                    ...
                    .fail(function (jqXHR, textStatus, errorThrown) {
                        if (c.debug) {
                            ts.log("aborting build table widget, failed ajax load");
                        }
    ...
    bt.build = {
        ...
        header: function (h1, wo) {
            var h2 = wo.build_headers.text,
                cls = wo.build_headers.classes,
                t = "<tr>" + (wo.build_numbers.addColumn ? "<th" + (wo.build_numbers.sortable ? "" : ' class="sorter-false"') + ">" + wo.build_numbers.addColumn + "</th>" : "");
            $.each(h1, function (i, h) {
                if (/<\s*\/t(d|h)\s*>/.test(h)) {
                    t += h;
                } else {
                    t += "<th" + (cls && cls[i] ? ' class="' + cls[i] + '"' : "") + ">" + (h2 && h2[i] ? h2[i] : h) + "</th>";
                }
            });
            return t + "</tr>";
        },
        rows: function (items, txt, c, wo, num, ftr) {
            ...
            $.each(items, function (i, item) {
                ...
                if (/<\s*\/t(d|h)\s*>/.test(item)) {
                    ...
                    t += "<" + (ftr ? h + (c && c[i] ? ' class="' + c[i] + '"' : "") : h) + ">" + (ftr && txt && txt.length && txt[i] ? txt[i] : item) + "</" + h + ">";
                }
            });
            return t + "</tr>";
        },
    ...
    bt.csv = function (table, data, wo) {
        ...
        $.each(lines, function (n, line) {
            ...
            if ((csv ? n >= wo.build_csvStartLine : true) && n < r) {
                ...
                if (infooter && f > 0) {
                    tableHTML += (n === len - f ? "</tbody><tfoot>" : "") + (n === len ? "</tfoot>" : "");
                }
                if (items.length > 1) {
                    printedLines++;
...
*/
    bt.object = function (table, data, wo) {
        ...
        if (!h || !r || h.length === 0 || r.length === 0) {
            if (c.debug) {
                ts.log("aborting build table widget, missing data for object build");
            }
            return false;
        }
