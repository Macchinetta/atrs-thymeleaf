        ...
        tablesorterPager: new (function () {
            ...
            var $this = this,
                ...
                pagerArrows = function (p, disable) {
                    ...
                    if (p.updateArrows) {
                        p.$container
                            .find(p.cssFirst + "," + p.cssPrev)
                            [first ? a : r](d)
                            .attr("aria-disabled", first);
                        p.$container
                            .find(p.cssNext + "," + p.cssLast)
                            [last ? a : r](d)
                ...
                updatePageDisplay = function (table, p, completed) {
                    ...
                    if (f && !p.ajaxUrl) {
                        if ($.isEmptyObject(c.cache)) {
                            ...
                            p.filteredRows = p.totalRows = c.$tbodies
                                .eq(0)
                                .children("tr")
                                .not(p.countChildRows ? "" : "." + c.cssChildRow).length;
                        } else {
                            p.filteredRows = 0;
                ...
                renderAjax = function (data, table, p, xhr, exception) {
                    ...
                    if (typeof p.ajaxProcessing === "function") {
                        ...
                        fixHeight(table, p);
                        $t.trigger("updateCache", [
                            function () {
                                if (p.initialized) {
                                    // apply widgets after table has rendered & after a delay to prevent
                                    // multiple applyWidget blocking code from blocking this trigger
                                    setTimeout(function () {
                                        $t.trigger("applyWidgets").trigger("pagerChange", p);
                                    }, 0);
                                }
                            },
                        ]);
                    ...
                    if (!p.initialized) {
                        p.initialized = true;
                        $(table).trigger("applyWidgets").trigger("pagerInitialized", p);
                    }
                },
                ...
                moveToPage = function (table, p, pageMoved) {
                    ...
                    if (p.initialized && pageMoved !== false) {
                        $t.trigger("pageMoved", p).trigger("applyWidgets");
                        if (table.isUpdating) {
                ...
                enablePager = function (table, p, triggered) {
                    var info,
                        c = table.config;
                    p.$size.add(p.$goto).add(p.$container.find(".ts-startRow, .ts-page")).removeClass(p.cssDisabled).removeAttr("disabled").attr("aria-disabled", "false");
                    p.isDisabled = false;
